...
m1=linspace(0, 0.2, 41);
k = linspace(0.00001, 0.1, 80);

for i = 1:length(m1)
    for j = 1:length(k)
        eqn = (M11(m1(i))+M12(m1(i))+M21(m1(i))+M22(m1(i)))*( k(j)^2)-1i*(calM1(m1(i))+calM2(m1(i)))*(k(j)) ...
                   - omg^2*(P+eps*(calM11(m1(i))*(M11(m1(i))\eye(3))+calM12(m1(i))*(M12(m1(i))\eye(3)) ...
                   +calM21(m1(i))*(M21(m1(i))\eye(3))+calM22(m1(i))*(M22(m1(i))\eye(3)))*P);
...
zlabel('$\omega$','interpreter','latex', 'FontSize',15);
